...
class WorstFitMemory extends JFrame {
    private int[] blocks;
    private int[] processes;
    private JTable table;
    private DefaultTableModel model;

    public WorstFitMemory(int[] blocks, int[] processes) {
        this.blocks = blocks;
        this.processes = processes;
        this.setTitle("Worst Fit Memory Allocation");
        ...
        this.model = new DefaultTableModel(columnNames, 0) {
            {
                Objects.requireNonNull(WorstFitMemory.this);
            }
        ...
        this.table.setSelectionBackground(new Color(255, 107, 166, 100));
    ...
    private void allocate() {
        ...
        int[] blockIndices = new int[blockCopy.length];

        for(int i = 0; i < blockCopy.length; blockIndices[i] = i++) {
        }

        for(int i = 0; i < blockCopy.length; ++i) {
            for(int j = i + 1; j < blockCopy.length; ++j) {
                if (blockCopy[i] < blockCopy[j]) {
                    int temp = blockCopy[i];
                    blockCopy[i] = blockCopy[j];
                    blockCopy[j] = temp;
                    int tempIndex = blockIndices[i];
                    blockIndices[i] = blockIndices[j];
                    blockIndices[j] = tempIndex;
                }
            }
        }
        ...
        for(int i = 0; i < this.processes.length; ++i) {
            ...
            for(int j = 0; j < blockCopy.length; ++j) {
                if (!blockUsed[j] && this.processes[i] <= blockCopy[j]) {
                    allocation[i] = blockIndices[j];
                    fragmentation[i] = blockCopy[j] - this.processes[i];
                    blockUsed[j] = true;
                    break;
                }
            }
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            ...
            new WorstFitMemory(blocks, processes);
        });
    }
}
