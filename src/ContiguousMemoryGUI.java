...
class ContiguousMemoryGUI extends JFrame {
    private int[] blocks;
    private int[] processes;
    int memoryblock;
    int processblock;

    public ContiguousMemoryGUI() {
        this.setTitle("Contiguous Memory Allocation");
        ...
        overlay.setBorder(BorderFactory.createEmptyBorder(50, 150, 70, 150));
        JPanel topBar = new JPanel(new BorderLayout());
        topBar.setOpaque(false);
        topBar.setBorder(BorderFactory.createEmptyBorder(0, 0, 30, 0));
        JLabel titleLabel = new JLabel("Contiguous Memory Allocation") {
            {
                Objects.requireNonNull(ContiguousMemoryGUI.this);
            }
        ...
        JPanel infoPanel = new JPanel(new GridLayout(2, 2, 20, 15));
        infoPanel.setOpaque(false);
        infoPanel.setBorder(BorderFactory.createEmptyBorder(0, 20, 30, 20));
        infoPanel.add(this.createInfoLabel("Memory Blocks: " + this.memoryblock));
        infoPanel.add(this.createInfoLabel("Processes: " + this.processblock));
        StringBuilder blockSizes = new StringBuilder("Blocks: ");

        for(int i = 0; i < this.blocks.length; ++i) {
            blockSizes.append(this.blocks[i]);
            if (i < this.blocks.length - 1) {
                blockSizes.append(", ");
            }
        }

        infoPanel.add(this.createInfoLabel(blockSizes.toString()));
        StringBuilder processSizes = new StringBuilder("Processes: ");

        for(int i = 0; i < this.processes.length; ++i) {
            processSizes.append(this.processes[i]);
            if (i < this.processes.length - 1) {
                processSizes.append(", ");
            }
        }

        infoPanel.add(this.createInfoLabel(processSizes.toString()));
        JPanel buttonPanel = new JPanel(new GridLayout(1, 3, 30, 30));
        buttonPanel.setOpaque(false);
        JButton firstFitBtn = this.createAlgorithmButton("First Fit", "1️⃣", new Color(88, 166, 255));
        JButton bestFitBtn = this.createAlgorithmButton("Best Fit", "\ud83c\udfaf", new Color(107, 255, 166));
        JButton worstFitBtn = this.createAlgorithmButton("Worst Fit", "\ud83d\udcca", new Color(255, 107, 166));
        buttonPanel.add(firstFitBtn);
        buttonPanel.add(bestFitBtn);
        buttonPanel.add(worstFitBtn);
        glassContainer.add(infoPanel, "North");
        glassContainer.add(buttonPanel, "Center");
        overlay.add(glassContainer, "Center");
        this.add(overlay);
        firstFitBtn.addActionListener((e) -> new FirstFitMemory(this.blocks, this.processes));
        bestFitBtn.addActionListener((e) -> new BestFitMemory(this.blocks, this.processes));
        worstFitBtn.addActionListener((e) -> new WorstFitMemory(this.blocks, this.processes));
        this.setVisible(true);
    }

    private void getInputData() {
        String input = JOptionPane.showInputDialog(this, "Please enter total memory blocks", "Memory Block Count", 3);
        if (input == null) {
            this.dispose();
            System.exit(0);
        }
        ...
        try {
            this.memoryblock = Integer.parseInt(input);
        ...
        this.blocks = new int[this.memoryblock];

        for(int i = 0; i < this.memoryblock; ++i) {
            String blockInput = JOptionPane.showInputDialog(this, "Enter size for Memory Block " + (i + 1) + ":", "Block " + (i + 1), 3);
            if (blockInput == null) {
                this.dispose();
                System.exit(0);
            }
            ...
            try {
                this.blocks[i] = Integer.parseInt(blockInput);
                if (this.blocks[i] <= 0) {
                    throw new NumberFormatException();
                }
        ...
        input = JOptionPane.showInputDialog(this, "Please enter total number of processes", "Process Count", 3);
        if (input == null) {
            this.dispose();
            System.exit(0);
        }
        ...
        try {
            this.processblock = Integer.parseInt(input);
            if (this.processblock <= 0) {
                throw new NumberFormatException();
            }
        ...
        this.processes = new int[this.processblock];

        for(int i = 0; i < this.processblock; ++i) {
            String processInput = JOptionPane.showInputDialog(this, "Enter size for Process " + (i + 1) + ":", "Process " + (i + 1), 3);
            ...
            try {
                this.processes[i] = Integer.parseInt(processInput);
    ...
    private JLabel createInfoLabel(String text) {
        ...
        label.setFont(new Font("Segoe UI", 1, 15));
        label.setForeground(new Color(255, 255, 255, 250));
        return label;
    }
    ...
    private JButton createAlgorithmButton(final String text, final String icon, final Color accentColor) {
