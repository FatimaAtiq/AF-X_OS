...
class DestroyProcess extends JFrame {
    ...
    public DestroyProcess() {
        this.setTitle("Destroy Process Manager");
    ...
    private JPanel createTitlePanel() {
        ...
        gbc.gridx = 0;
        JLabel title = new JLabel("Destroy Process Manager");
        ...
        this.pidField.addActionListener((e) -> this.destroyProcess());
        gbc.gridy = 2;
        panel.add(this.pidField, gbc);
        JButton destroyBtn = this.createButton("Destroy Process", new Color(241, 66, 66));
        destroyBtn.addActionListener((e) -> this.destroyProcess());
        gbc.gridy = 3;
        panel.add(destroyBtn, gbc);
        return panel;
    }
    ...
    private JPanel createSearchPanel() {
        ...
        this.searchField.addKeyListener(new KeyAdapter() {
            {
                Objects.requireNonNull(DestroyProcess.this);
            }

            public void keyReleased(KeyEvent e) {
                DestroyProcess.this.filterTable(DestroyProcess.this.searchField.getText());
            }
        });
    ...
    private void styleTable() {
        ...
        this.table.getColumnModel().getColumn(5).setCellRenderer(new DefaultTableCellRenderer() {
            ...
            public Component getTableCellRendererComponent(JTable t, Object v, boolean s, boolean f, int r, int c) {
                ...
                if (!s) {
                    lbl.setBackground(DestroyProcess.TABLE_BG);
                    if ("Running".equals(v)) {
                        lbl.setForeground(new Color(40, 167, 69));
                    } else if ("Blocked".equals(v)) {
    ...
    private void destroyProcess() {
        try {
            ...
            if (p == null) {
                this.msg("Process not found", "Error", 0);
            } else if (!"Running".equals(p.status)) {
                this.msg("Only Running processes can be destroyed", "Warning", 2);
            } else {
                String processName = p.name;
                createprocess.processList.remove(p);
                this.msg("Process '" + processName + "' (PID: " + pid + ") destroyed successfully!", "Success", 1);
                this.pidField.setText("");
                this.refresh();
            }
        } catch (NumberFormatException var4) {
            this.msg("Invalid PID. Please enter a valid number.", "Error", 0);
        }
    ...
    private void filterTable(String q) {
        ...
        if (createprocess.processList != null) {
            for(Process p : createprocess.processList) {
                if ("Running".equals(p.status) && (p.name.toLowerCase().contains(q) || p.status.toLowerCase().contains(q) || String.valueOf(p.id).contains(q))) {
                    this.tableModel.addRow(new Object[]{p.id, p.name, p.priority, p.arrivalTime, p.burstTime, p.status});
                }
            }
    ...
    private void updateStatus() {
        ...
        int totalCount = 0;
        if (createprocess.processList != null) {
            totalCount = createprocess.processList.size();
        ...
        this.statusLabel.setText("Running Processes: " + runningCount + " | Total Processes: " + totalCount);
    }
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(DestroyProcess::new);
    }
